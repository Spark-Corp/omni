    ...
    <div className="min-h-screen bg-neutral-950 text-white">
      ...
      <div className="sticky top-0 z-20 bg-neutral-950/80 backdrop-blur-xl border-b border-white/10">
        <div className="max-w-2xl mx-auto px-4 py-4 flex items-center gap-3">
          <Link
            to="/map"
            className="w-8 h-8 rounded-full bg-white/5 hover:bg-white/15 border border-white/10 flex items-center justify-center"
          >
          ...
          {carts.length > 0 && (
            <span className="text-xs text-white/30 ml-auto">
              {carts.length} commande{carts.length > 1 ? "s" : ""}
            </span>
          )}
        </div>
      </div>
        ...
        {loading ? (
          ...
          <div className="text-center py-20">
            ...
            <p className="text-white/10 text-xs mt-1">
              Envoie un panier depuis la carte pour voir ton historique
            </p>
            <Link
              to="/map"
              className="inline-block mt-6 px-6 py-2.5 rounded-full bg-emerald-500 text-black text-sm font-medium hover:bg-emerald-400 transition-all"
            >
              Voir la carte
            </Link>
          </div>
            ...
            {carts.map((cart) => (
              ...
              >
                ...
                <div className="px-4 py-3 bg-white/[0.03] border-b border-white/5 flex items-center justify-between">
                  ...
                  <span
                    className={`text-[10px] px-2 py-1 rounded-full border font-medium shrink-0 ml-3 ${STATUS_COLORS[cart.status] || STATUS_COLORS.pending}`}
                  >
                    ...
                    >
                      <div className="flex-1 min-w-0 mr-3">
                        ...
                        <p className="text-white/30 text-xs mt-0.5">
                          ...
                          {item.product_price?.toLocaleString()} FCFA
                        </p>
                      </div>
                        ...
                        onClick={async () => {
                          try {
                            ...
                            const res = await fetch(
                              `/api/cart/${cart.id}/received`,
                              {
                                method: "POST"
                              }
                            );
                            if (res.ok) {
                              toast("Commande marquée reçue");
                              fetchCarts();
                            } else {
                              toast("Erreur");
                            }
      ...
      {chatRequest && (
        <ChatModal
          ...
          vendorName={
            carts.find((c) => c.items[0]?.product_id === chatRequest.product_id)
              ?.vendor_name || "Vendeur"
          }
