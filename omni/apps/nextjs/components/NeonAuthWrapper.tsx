...
/**
 * NeonAuthWrapper - Isolates Neon Auth UI in an iframe to avoid Next.js SSR conflicts
 * 
    ...
    return (
      <div className='p-6 bg-red-500/10 border border-red-500/30 rounded-xl text-red-300'>
        ...
        <p className='text-sm mt-2'>
          Please add NEXT_PUBLIC_NEON_AUTH_URL to your .env file
        </p>
      </div>
    );
  }
...
export function SimpleAuthWrapper({ 
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (result.error) {
        setError(result.error.message || (isSignUp ? 'Erreur lors de l inscription' : 'Email ou mot de passe incorrect'));
      } else {
